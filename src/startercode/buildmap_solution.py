'''buildmap_solution.py

   This is the solution code for building a map.
...
class CustomNode(Node):
    ...
    def laserCB(self, msg):
        ...
        #  P5:               Just leave a trace.
        #  P6:               Just mark the endpoints.
        #  P8 and following: Update the occupancy grid.
        if False:
            self.markrobot(xc, yc)
        elif False:
            self.markendpoints(xc, yc, thetac, thetas, ranges, rmin, rmax)
        else:
            self.updateoccupancy(xc, yc, thetac, thetas, ranges, rmin, rmax)


    # Mark the robot's (lidar's) position to leave a trace.
    def markrobot(self, xc, yc):
        # Convert to pixels.
    ...
    # Mark the ray's endpoints, which should overlap with scans in RVIZ.
    def markendpoints(self, xc, yc, thetac, thetas, ranges, rmin, rmax):
        # Process each ray (at a different angle)
        for (theta,r) in zip(thetas, ranges):
            # Make sure the reading has a valid contact.
            if (r <= rmin) or (r >= rmax):
                continue

            # Determine the endpoint.
            x = xc + r * np.cos(thetac + theta)
            y = yc + r * np.sin(thetac + theta)
    ...
    def updateoccupancy(self, xc, yc, thetac, thetas, ranges, rmin, rmax):
        ...
        for (theta,r) in zip(thetas, ranges):
            ...
            rend = min(r, rmax)
            us = (xc + rmin*c - ORIGIN_X) / RESOLUTION
            vs = (yc + rmin*s - ORIGIN_Y) / RESOLUTION
            ue = (xc + rend*c - ORIGIN_X) / RESOLUTION
            ve = (yc + rend*s - ORIGIN_Y) / RESOLUTION
